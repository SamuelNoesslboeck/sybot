...
{   
    ...
    // Calculation
        // Position
            ...
            fn phis_from_vec(&self, pos : Vec3, dec_ang : f32) -> Phis<4> {
                let pos_def = self.reduce_to_def(pos, dec_ang);
                let mut phis = self.phis_from_def_vec(pos_def);
                phis[3] = dec_ang - (phis[1] + phis[2]);
                phis
            }
        //

        // Load
            fn inertias_from_vecs(&self, vecs : &Vectors<4>) -> Inertias<4> {
            ...
            fn forces_from_vecs(&self, vecs : &Vectors<4>) -> Forces<4> {
        ...
        fn update(&mut self, phis : Option<&Phis<4>>) {
    ...
    // Actions
        fn measure(&mut self, acc : u64) -> Result<(), [bool; 4]> {
            let [ _, res_1, res_2, res_3 ] = self.comps.measure(self.mach.meas_dist, self.mach.vels, 
                self.mach.meas.iter().map(|meas| meas.set_val).collect::<Vec<f32>>().try_into().unwrap(), [acc; 4]);
